...
namespace OpenXLivePages
{
    ...
    /// </summary>
    public sealed partial class GamesListPage : OpenXLivePages.Common.LayoutAwarePage
    {
        ...
        public GamesListPage()
        {
            ...
            pgb.VerticalAlignment = Windows.UI.Xaml.VerticalAlignment.Bottom;
        ...
        private async void LoadData()
        {
            ...
            {
                GameSearch search = new GameSearch(session);

                try
                {
                    AsyncProcessResult result = await search.GetGameListAsync();

                    if (result.ReturnValue)
                    {
                        var group = new OpenXLiveDataGroup("GamesList", "GamesList", "", "", "");
                        var list = search.GameList;

                        foreach (MyGame game in list)
                        {
                            group.Items.Add(new OpenXLiveDataItem(
                                game.GameID,
                                game.Name,
                                game.Developer,
                                game.Image.Url.ToString(),
                    ...
                    {
                        MessageDialog dlg = new MessageDialog(result.ErrorMessage);
                        await dlg.ShowAsync();
                    }
                }
                catch (Exception ex)
